...
class DashboardController extends Controller {
    ...
    public function order_show($order_id) {
        $order = Order::with('user')->where('order_id', $order_id)->firstOrFail();
	...
	public function digitalProducts() {
        $digital_products = OrderProduct::select('order_products.*', 'products.image', 'products.slug')
		->join('products', 'products.id', '=', 'order_products.product_id')
		->join('orders', 'orders.id', '=', 'order_products.order_id')
		->where('products.type', 'digital')
		->where('orders.user_id', userAuth()->id)
		->latest()->paginate(10);
