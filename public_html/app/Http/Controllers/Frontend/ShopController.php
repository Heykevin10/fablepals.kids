...
class ShopController extends Controller {
	public function shop(Request $request)
	{
		...
		$query = Product::select('id', 'slug', 'image', 'price', 'qty', 'sale_price', 'is_new')
			->with([
				'favoritedBy',
		...
		$from = $request->filled('from') ? $request->from : 0;
		$to = $request->filled('to') ? $request->to : $max_price;
		...
		// Pagination
		$product_per_age = cache('CustomPagination')?->product_list 
			?? CustomPagination::where('section_name', 'Product List')->value('item_qty');
		...
		// Return full view instead of JSON
		return view('frontend.pages.shop.index', compact(
			'products',
			'categories',
			'topTags',
			'breadcrumb_result',
    ...
    public function singleProduct($slug): View {
        $product = Product::select('id', 'product_category_id', 'slug','type', 'sku', 'price','qty','sale_price', 'is_new', 'tags', 'image', 'file_path')->with(['favoritedBy',
            'translation'          => function ($query) {
		...
		$product_files = DB::table('product_files')
		...
		$youtubeStats = DB::table('youtube_stats')->first();

        return view('frontend.pages.shop.single-product', compact('youtubeStats','product', 'canReview','tagString','relatedProducts', 'product_files', 'fileCounts'));
    }
