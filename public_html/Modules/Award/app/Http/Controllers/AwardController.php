...
class AwardController extends Controller {
    ...
    public function index() {
        ...
        return view('award::index', compact('awards', 'languages', 'code', 'border_color'));
    }
	...
	public function colorSave(Request $request){
        ...
        if(DB::table('award_colors')->where('id', 1)->exists()){
			DB::table('award_colors')->where('id', 1)->update(['color' => $request->color]);
		}else{
			DB::table('award_colors')->insert(['color' => $request->color]);
		}
		
		return redirect()->to( route('admin.award.index', ['code' => getSessionLanguage()]));
