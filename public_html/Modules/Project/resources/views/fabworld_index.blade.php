...
@section('admin-content')
    <div class="main-content">
        <section class="section">
            <x-admin.breadcrumb title="{{ __('Character List') }}" :list="[
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <x-admin.form-title :text="__('Character List')" />
                                <div>
                                    <x-admin.add-button :href="route('admin.fabworld.create')" />
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            @forelse ($projects as $project)
                                                <tr>
                                                    <td>{{ $loop->index + 1 }}</td>
                                                    <td><img src="{{ asset($project->banner) }}"
                                                            alt="{{ $project->title }}" class="rounded-circle my-2"></td>
                                                    <td>{{ $project->banner_heading ?? '' }}</td>
                                                    @adminCan('project.management')
                                                        <td>
                                                            <input class="change-status" data-href="{{route('admin.fabworld.status-update',$project->id)}}"
                                                        ...
                                                        <td>
														<a href="{{ route('fabworld.profile', $project->slug) }}" class="btn btn-info btn-sm" target="_blank">
															<i class="fa fa-eye" aria-hidden="true"></i>
														</a>
                                                            <x-admin.edit-button :href="route('admin.fabworld.edit', [
                                                                'project' => $project->id,
                                                                'code' => getSessionLanguage(),
                                                            ])" />
                                                                <a href="{{ route('admin.fabworld.destroy', $project->id) }}"
                                            ...
                                            @empty
                                                <x-empty-table :name="__('Project')" route="admin.fabworld.create" create="yes"
