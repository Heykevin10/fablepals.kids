...
class ProductController extends Controller {
    ...
    public function create() {
		...
		$fabworlds = DB::table('fabworlds')->select('title', 'id', 'banner_heading')->where('status', 1)->get();
		$characters = DB::table('characters')->select('character_translations.title', 'characters.id')
		->join('character_translations', 'character_translations.character_id', '=', 'characters.id')
		->where('characters.status', 1)->get();

        return view('shop::Product.create', ['categories' => $categories, 'type' => $type, 'is_digital' => $is_digital, 'fabworlds' => $fabworlds, 'characters' => $characters]);
    }
    ...
    public function store(ProductRequest $request): RedirectResponse {
		...
		if ($product && $request->hasFile('product_files')) {

    $uploadPath = public_path('uploads/product-files/');
    ...
    foreach ($request->file('product_files') as $file) {
			...
			if ($file->isValid()) {
				...
				// Determine file type
				$fileType = in_array($extension, ['jpg','jpeg','png','gif','webp']) ? 'image' : 'pdf';
				...
				DB::table('product_files')->insert([
					...
					'file_path'  => 'uploads/product-files/' . $fileName,
					'file_type'  => $fileType,
					'created_at' => now(),
					'updated_at' => now(),
				]);
		...
		$model->fabworld_id = $request->fabworld_id;
		$model->character_id = $request->character_id;
		$model->save();
    ...
    public function edit($id) {
		...
		$fabworlds = DB::table('fabworlds')->select('title', 'id')->where('status', 1)->get();
		$characters = DB::table('characters')->select('character_translations.title', 'characters.id')
		->join('character_translations', 'character_translations.character_id', '=', 'characters.id')
		->where('characters.status', 1)->get();

        return view('shop::Product.edit', compact('product', 'code', 'categories', 'languages', 'is_digital', 'fabworlds', 'characters'));
    }
	...
	public function deleteFile(Request $request)
	{
		...
		if ($file) {
			...
			// Delete DB record
			DB::table('product_files')->where('id', $request->id)->delete();
	...
	{
		$file_path = DB::table('products')->where('id', $request->id)->value('file_path');
		...
		if ($file_path) {
			...
			DB::table('products')->where('id', $request->id)->update(['file_path' => NULL]);
