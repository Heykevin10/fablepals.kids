...
<style>
  ...
  /* Only remove grey background */
  .breadcrumb-wrapper.style2,
    ...
    <section class="shop__area space">
        <div class="container">
            <div class="row gx-60 gy-60">
                ...
                <div class="col-xl-6">
				<!--<h1><button onclick="startCheckout()">Buy Now</button></h1>-->
                    <div class="product-about">
                        <h3 class="product-title" style="font-size: 30px;">{{ $product?->title }}</h3>
                        ...
                        <div class="actions">
                            ...
                            @else
                                @if ($product?->qty)
									...
									{{-- route('user.buy.now', ['slug' => $product?->slug]) --}}
                                    <a href="javascript:;" class="btn style2 buy-now-btn"
                                        data-url="{{ route('buy.now', $product->slug) }}?qty=1&buy_now=1">
                                        <span class="link-effect">
									...
									<a href="https://www.youtube.com/@FablePalsTV" target="_blank" class="btn btn-danger" style=" background: #FF0000!important;">
										Subscribe
									</a>
                                @else
						...
						if(!empty($product->file_path)){
							$file_type = explode('.', $product->file_path);
							$lastItem = end($file_type);
						}
        ...
        <span class="effect-1"><i class="fab fa-whatsapp"></i></span>
            ...
            @if ($relatedProducts->count())
                <div class="space-top">
                    ...
                    <div class="row global-carousel related-products" data-slide-show="3" data-md-slide-show="2" data-sm-slide-show="2">
                        @foreach ($relatedProducts as $product)
                            <div class="col-sm-6">
                                <div class="product-card">
                                    <div class="product-img">
                                        <a href="{{ route('single.product', $product?->slug) }}" style="position: relative;z-index: 9;">
                                        ...
                                        @endif
										@if ($product?->qty)
											<a href="javascript:;" class=" wsus-wishlist-btn btn add-to-cart-btn"
...
@push('js')
    <script>
	function whatsApp() {
		let text = "{{ $product->name }} - " + window.location.href;
...
function startCheckout() {
    fetch("{{url('api/shiprocket/checkout')}}", {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            ...
            items: [
                { id: 1, price: 499, qty: 1 },
                { id: 2, price: 299, qty: 2 }
            ]
        })
    })
    .then(res => res.json())
