                        ...
                        @forelse ($userFavorites as $product)
                            <tr>
                                ...
                                <td class="details">
                                    ...
                                    $regular = $product->regular_price ?? $product->old_price ?? $product->price;
                                    $sale    = $product->offer_price ?? $product->sale_price ?? null;
                                
                                    
                                    if (is_null($sale) && $product->price && $regular && $product->price < $regular) {
                                        $sale = $product->price;
                                    }
                                
                                    $finalPrice = $sale ?? $product->price;
                                @endphp
                                
                                @if($sale)
                                    <del>{{ currency($regular) }}</del>
                                    <span class="ms-2">{{ currency($sale) }}</span>
                                @else
