...
<section class="position-relative text-white" style="min-height: 700px;">
	<div class="w-100 h-100 position-absolute top-0 start-0 z-n1"
		 style="background: url('https://fablepals.kids/uploads/custom-images/image_1754125032.webp') no-repeat center center; background-size: cover;">
	</div>
			...
			<div class="row justify-content-center">
				@foreach ($projects as $service)
					...
					if (!empty($service->line_color)) {
						$color_arr = explode(',', $service->line_color);
						$color_one = trim($color_arr[0]) ?? '';
					}else{
						$color_one = '';
					}
					@endphp
					<div class="col-md-4 p-4">
						<div class="feature-card"
							...
							<h4 class="feature-card-title">
								<a href="{{ route('character.profile', $service?->slug) }}">{{ $service?->title }}</a>
							</h4>
							<p class="feature-card-text"></p>
							<a href="{{ route('character.profile', $service?->slug) }}" class="link-btn">
